...
async function templateToHtml({ file, name, logoDataUri, logoCompanyName }) {
  ...
  const instruction = [
    ...
    "LAYOUT FIDELITY: keep each label, its input, and any trailing unit/suffix",
    "(e.g. '(inches)', '(lbs)', '%') together so they NEVER break across lines —",
    "wrap each label+input+unit cluster in a single inline-block container with",
    "style='white-space:nowrap; display:inline-block;'. Reproduce the original's row",
    "structure: fields that appear on the same horizontal line of the original form",
    "must stay on one line (use a flex row, e.g. display:flex; gap; flex-wrap:nowrap),",
    "and only start a new line where the original form starts a new line. Do not let a",
    "unit detach from its input or a label detach from its field.",
