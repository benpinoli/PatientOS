-- Replace patients.external_code with birth_date; update patient creation RPC.
...
drop function if exists public.create_patient_with_tasks(text, text, text, text, uuid, uuid, uuid, int);
...
begin
  ...
  if nullif(btrim(p_first_name), '') is null
    ...
    raise exception 'First name, last name, birth date, and payer are required.' using errcode = '23502';
  end if;
...
grant execute on function public.create_patient_with_tasks(
  text, text, date, text, uuid, uuid, uuid, int
) to authenticated;
